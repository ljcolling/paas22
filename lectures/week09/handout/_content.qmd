...
**difference** is just `r ama_mean`ms - `r spo_mean`ms = `r ama_mean -
spo_mean`ms. Because this difference is positive, the amateurs have a
...
our sample and the sampling distribution. Last we learned about the sampling
...
set.seed(123)
n <- 50
face_celeb <- rnorm(n, 506, 50)
face_family <- rnorm(n, 650, 45)
subject_id <- seq(1, n)
...
100 ms, or 50 ms, or 0 ms. Once we have hypothesised a value, we can generate a
...
```{r}
#| label: fig-sampling
#| fig-cap: The sampling distribution with a mean of 0 and a SEM of 8.8
require(tidyverse)
...
We can now compare out particular sample mean of `r sum_stats$mean_rt |> round(2)`ms 
to this sampling distribution. Because the sampling distribution is a normal distribution
we known that ~68% of the time the sample means will fall between ±1 SEM of the population
mean. Or, between -8.8 ms and 8.8ms. And ~90% of the time they'll fall between
