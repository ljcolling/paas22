...
- **Operationalisation** means choosing a **measurable** proxy for the
  phenomenon you're interested in^[_Operationalisation_ will be an important
...
- Involves testing **predictions** that logically follow from **theories** (the
  _deductive_ step)^[Refer back Lecture 2 if you're unsure what _deductive_
  means]
...
Qualitative methods are extremely varied with many different methodologies,
...
#| tbl-label: tbl-vpa
readr::read_csv("./vpa_example.csv") |>
  knitr::kable()
```
...
knitr::include_graphics("sexy_chart.png")
```
...
knitr::include_graphics("./gol.gif")
```
