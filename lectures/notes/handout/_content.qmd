...
require(dplyr)
require(plotly)
salary <- readr::read_csv("world_salary.csv")
...
(_sequences_ will make more sense when we add more coins) than can happen when
we flip a coin.
...
that produces one head. And those are all the possible sequences.
